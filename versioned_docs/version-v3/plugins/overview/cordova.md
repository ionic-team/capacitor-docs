...
Install the plugin, sync, and then finish any required native project configuration (see [Variables and Hooks](/docs/v3/plugins/cordova#variables-and-hooks)):
...
> If the Cordova plugin has an [Ionic Native](https://ionicframework.com/docs/v3/native) wrapper, you can also install it for TypeScript support:
>
> ```bash
...
There may be compatibility issues with Capacitor and some Cordova plugins. Many of the official Cordova plugins should not be used, as Capacitor offers [official alternatives](/docs/v3/apis). Cordova plugins that use variables and hooks may be partially compatible. Some Cordova plugins are completely incompatible (see [this list](/docs/v3/plugins/cordova#known-incompatible-plugins)).
...
Consult the [iOS](/docs/v3/ios/configuration) and [Android](/docs/v3/android/configuration) configuration guides for info on how to configure each platform.
